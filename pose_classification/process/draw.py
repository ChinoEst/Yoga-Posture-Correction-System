...
def draw(csv):
    ...
    #img = cv2.imread(f"{file_name}.jpg")
    
    keypoint = {}
    for i in range(23):
        #draw point
        x = int(df[f"{keypoints_columns[2*i]}"].iloc[0])
        y = int(df[f"{keypoints_columns[2*i+1]}"].iloc[0])
    ...
    cv2.imwrite(f"{file_name}_skeleton.jpg", img)
    print(file_name)
...
whos = ['assistant', 'dean', 'david', 'father', 'harvey', 'jimmy', 'kevin','me', 'wei', 'yee']
sides = ["side"]
poses = [str(i) for i in range(1, 11)]
...
for side in sides:
    for i in poses:
        for who in whos:
            temp_pth = os.path.join("..", "data", who, side, i, "augmentation")
            if os.path.exists(temp_pth):
                files = [os.path.join(temp_pth, file) for file in os.listdir(temp_pth) if file.endswith('.csv') ]
